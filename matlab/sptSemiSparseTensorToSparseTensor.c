...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    spt_mxCheckArgs("sptSemiSparseTensorToSparseTensor", 1, "One", 2, "Two");

    sptSemiSparseTensor *src = spt_mxGetPointer(prhs[0], 0);
    sptScalar epsilon = mxGetScalar(prhs[1]);

    sptSparseTensor *dest = malloc(sizeof *dest);
    int result = sptSemiSparseTensorToSparseTensor(dest, src, epsilon);
    if(result) {
        free(dest);
        dest = NULL;
    }
