...
int main() {
    ...
    short **dev_data;
    ptiAssert(ptiCudaDuplicateMemoryIndirect(&dev_data, header, 7, length, cudaMemcpyHostToDevice) == 0);

    print_dev_data("Device data", dev_data);

    short **copyback_data;
    ptiAssert(ptiCudaDuplicateMemoryIndirect(&copyback_data, dev_data, 7, length, cudaMemcpyDeviceToHost) == 0);
