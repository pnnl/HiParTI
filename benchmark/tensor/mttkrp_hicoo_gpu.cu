...
int main(int argc, char ** argv) {
    ...
    if (renumber == 1) {
        ptiIndex ** map_inds = (ptiIndex **)malloc(tsr.nmodes * sizeof *map_inds);
        ...
        // printf("map_inds:\n");
        // for(ptiIndex m = 0; m < tsr.nmodes; ++m) {
        //     ptiDumpIndexArray(map_inds[m], stdout);
        // }
        // ptiAssert(ptiDumpSparseTensor(&tsr, 0, stdout) == 0);
    ...
    if(cuda_dev_id == -2) {
        ...
        ptiAssert(ptiCudaMTTKRPHiCOO(&hitsr, U, mats_order, mode, max_nnzb, impl_num) == 0);
    }
