...
{
    if(tk > 1 && tb == 1) {
        sptAssert(sptOmpMTTKRPHiCOOKernels_MatrixTiling_Scheduled_Reduce_Two(hitsr, mats, copy_mats, mats_order, mode, tk) == 0);
    } else {
        printf("Haven't support block parallelism.\n");
        return -1;
    }
    ...
    const int tk) 
{
    ...
    if(nmodes == 3) {
        sptAssert(sptOmpMTTKRPHiCOOKernels_3D_MatrixTiling_Scheduled_Reduce_Two(hitsr, mats, copy_mats, mats_order, mode, tk) == 0);
        return 0;
    }
