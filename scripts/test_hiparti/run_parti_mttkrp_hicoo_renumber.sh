...
out_path="/people/liji541/Codes/HiParTI-dev/timing-results/hicoo/renumber/uint8-single-11272019"
...
do
	...
	do
		...
		# echo "./build/benchmark/mttkrp_hicoo_renumber_matrixtiling -i ${tsr_path}/${tsr_name}_nnz.tns -b ${sb} -k ${sk} -c ${sc} -d ${dev_id} -r ${R} -e ${renum} -n ${niters_renum} > ${out_path}/${tsr_name}-b${sb}-k${sk}-c${sc}-r${R}-e${renum}-n${niters_renum}-mattile-seq.txt"
		# ./build/benchmark/mttkrp_hicoo_renumber_matrixtiling -i ${tsr_path}/${tsr_name}_nnz.tns -b ${sb} -k ${sk} -c ${sc} -d ${dev_id} -r ${R} -e ${renum} -n ${niters_renum} > ${out_path}/${tsr_name}-b${sb}-k${sk}-c${sc}-r${R}-e${renum}-n${niters_renum}-mattile-seq.txt
		...
		if [ ${tsr_name} = "freebase_music" ] || [ ${tsr_name} = "freebase_sampled" ]; then
			sk=16	# Optimal for lexiorder
			# sk=18
		fi
		if [ ${tsr_name} = "flickr" ]; then
			sk=13	# Optimal for lexiorder
			# sk=15
		fi
		...
		if [ ${tsr_name} = "flickr-4d" ]; then
			sk=12	# Optimal for lexiorder
			# sk=15
		fi
		if [ ${tsr_name} = "delicious-4d" ]; then
			sk=15	# Optimal for lexiorder
			# sk=16
		fi
		...
		# do
			...
			echo "./build/benchmark/mttkrp_hicoo_renumber_matrixtiling -i ${tsr_path}/${tsr_name}_nnz.tns -b ${sb} -k ${sk} -c ${sc} -d ${dev_id} -r ${R} -t ${tk} -l ${tb} -e ${renum} -n ${niters_renum} > ${out_path}/${tsr_name}-b${sb}-k${sk}-c${sc}-r${R}-e${renum}-n${niters_renum}-tk${tk}-tb${tb}-mattile.txt"
			./build/benchmark/mttkrp_hicoo_renumber_matrixtiling -i ${tsr_path}/${tsr_name}_nnz.tns -b ${sb} -k ${sk} -c ${sc} -d ${dev_id} -r ${R} -t ${tk} -l ${tb} -e ${renum} -n ${niters_renum} > ${out_path}/${tsr_name}-b${sb}-k${sk}-c${sc}-r${R}-e${renum}-n${niters_renum}-tk${tk}-tb${tb}-mattile.txt
